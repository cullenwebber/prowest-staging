(() => {
        ...
        btn.addEventListener("click", () => {
          ...
          if (document.body.classList.contains("menu-open")) {
            staggerContainers.forEach((stagger) => {
              const elementsStagger = gsapWithCSS.utils.toArray(stagger.children);
              gsapWithCSS.from(elementsStagger, {
                y: 20,
                opacity: 0,
                delay: 0.25,
                stagger: 0.1
  ...
  function ScrollContain(viewSize, contentSize, snapsAligned, containScroll, pixelTolerance) {
    ...
    function measureBounded() {
      return snapsAligned.map((snapAligned, index2) => {
        ...
        if (isLast)
          return min;
        if (usePixelTolerance(min, snap3))
          return min;
        if (usePixelTolerance(max, snap3))
          return max;
        return snap3;
  ...
  function ScrollSnaps(axis, alignment, containerRect, slideRects, slidesToScroll) {
    ...
    function measureAligned() {
      return groupSlides(snaps).map((g) => g[0]).map((snap3, index2) => snap3 + alignments[index2]);
    }
    const self = {
      snaps,
      snapsAligned
  ...
  function ScrollTarget(loop, scrollSnaps, contentSize, limit, targetVector) {
    ...
    function findTargetSnap(target) {
      ...
      const ascDiffsToSnaps = scrollSnaps.map((snap3, index3) => ({
        diff: shortcut(snap3 - distance, 0),
        index: index3
    ...
    function byDistance(distance, snap3) {
  ...
  function SlideLooper(axis, viewSize, contentSize, slideSizes, slideSizesWithGaps, snaps, scrollSnaps, location, slides) {
    ...
    function findSlideBounds(offset) {
      return snaps.map((snap3, index2) => ({
        start: snap3 - slideSizes[index2] + roundingSafety + offset,
  ...
  function Engine(root, container, slides, ownerDocument, ownerWindow, options, eventHandler) {
    ...
    const animation = Animations(ownerDocument, ownerWindow, (timeStep) => update(engine, timeStep), (lagOffset) => render3(engine, lagOffset));
    const friction = 0.68;
  ...
  function AutoScroll(userOptions = {}) {
    ...
    function init4(emblaApiInstance, optionsHandler) {
  ...
  // js/components/sliders/arrowButtons.js
  ...
  // js/components/sliders/dotButtons.js
  var addDotBtnsAndClickHandlers = (emblaApi, dotsNode) => {
    ...
    const addDotBtnsWithClickHandlers = () => {
      ...
      dotNodes.forEach((dotNode, index2) => {
        dotNode.addEventListener("click", () => scrollTo(index2), false);
      });
    };
  ...
  // js/components/sliders/sliders.js
  ...
  function animateOnScroll() {
    ...
    elementsStagger.forEach((stagger) => {
      ...
      const elementsStagger2 = gsapWithCSS.utils.toArray(stagger.children);
      gsapWithCSS.from(elementsStagger2, {
        scrollTrigger: {
          trigger: stagger,
          start: "top bottom-=10%"
        },
  ...
  // js/components/capabilityTimerAccordion.js
  var index = 0;
  var timeoutId = null;
  var progressAnimation = null;
  var SECTOR_DURATION = 1e4;
  function capabilityTimerAccordion() {
    const sectors = document.querySelectorAll("[capability-sector]");
    const backgroundImages = document.querySelectorAll(".sector-bg-image");
    if (sectors.length === 0)
      return;
    sectors[0].classList.add("active");
    backgroundImages[0].classList.add("active");
    startProgressBar(sectors[0]);
    scheduleNext(sectors, backgroundImages);
    addClickHandlers();
  }
  function scheduleNext(sectors, backgroundImages) {
    timeoutId = setTimeout(() => {
      changeActive(sectors, backgroundImages);
      scheduleNext(sectors, backgroundImages);
    }, SECTOR_DURATION);
  }
  function changeActive(sectors, backgroundImages) {
    if (progressAnimation) {
      progressAnimation.kill();
    }
    sectors.forEach((el) => {
      el.classList.remove("active");
      const progressBar = el.querySelector(".percentage-inner");
      if (progressBar) {
    ...
    startProgressBar(sectors[index]);
  }
  function startProgressBar(activeSector) {
    const progressBar = activeSector.querySelector(".percentage-inner");
    if (!progressBar)
      return;
    gsapWithCSS.set(progressBar, { width: "0%" });
    progressAnimation = gsapWithCSS.to(progressBar, {
      width: "100%",
      duration: SECTOR_DURATION / 1e3,
      ease: "none"
    });
  }
  function addClickHandlers() {
    const sectors = document.querySelectorAll("[capability-sector]");
    const backgroundImages = document.querySelectorAll(".sector-bg-image");
    sectors.forEach((sector, clickedIndex) => {
      sector.addEventListener("click", () => {
        ...
        sectors.forEach((s, i) => {
          ...
          const progressBar = s.querySelector(".percentage-inner");
          if (progressBar) {
            gsapWithCSS.set(progressBar, { width: "0%" });
          }
        });
  ...
  // js/components/service-accordion.js
  function initServiceAccordion() {
    const accordions = document.querySelectorAll(".service-accordion");
    const images = document.querySelectorAll(".service-image");
    if (!accordions.length || !images.length) {
      return;
    }
    images.forEach((img, index2) => {
      img.style.opacity = index2 === 0 ? "1" : "0";
      img.style.transition = "opacity 0.5s ease";
    });
    accordions.forEach((accordion, index2) => {
      accordion.addEventListener("click", () => {
        ...
        if (images[index2]) {
          images[index2].style.opacity = "1";
        }
      });
    });
  }

  // js/utils/parallax-effect.js
  gsapWithCSS.registerPlugin(ScrollTrigger2);
  var ParallaxImageEffect = class {
    constructor() {
      this.elements = [];
      this.rafId = null;
    ...
    init() {
      if (this.isInitialized)
        return;
      const containers = document.querySelectorAll("[parallax-image]");
      if (containers.length === 0) {
        console.warn("No parallax-image elements found");
        return;
      }
      containers.forEach((container, index2) => {
        this.setupParallaxElement(container, index2);
      });
      this.setupResizeHandler();
      this.isInitialized = true;
      ScrollTrigger2.refresh();
    }
    setupParallaxElement(container, index2) {
      const img = container.querySelector("img");
      if (!img) {
        console.warn("No image found in parallax container", container);
        return;
      }
      img.style.willChange = "transform";
      this.waitForImageLoad(img).then(() => {
        this.createParallaxAnimation(container, img, index2);
      });
    }
    ...
    createParallaxAnimation(container, img, index2) {
      gsapWithCSS.set(img, {
        scale: 1.2,
    ...
    destroy() {
      ...
      this.elements.forEach(({ img, timeline: timeline2 }) => {
        img.style.willChange = "auto";
        if (timeline2) {
          timeline2.kill();
        }
        gsapWithCSS.set(img, { clearProps: "all" });
      });
      ...
      ScrollTrigger2.getAll().forEach((trigger) => {
        if (trigger.vars.trigger && trigger.vars.trigger.hasAttribute("parallax-image")) {
          trigger.kill();
        }
      });
      this.elements = [];
    ...
    static create() {
      const instance = new ParallaxImageEffect();
      if (document.readyState === "loading") {
        document.addEventListener("DOMContentLoaded", () => instance.init());
      } else {
        requestAnimationFrame(() => instance.init());
      }
      return instance;
    }
  };
  var parallaxEffect = () => ParallaxImageEffect.create();
  var parallax_effect_default = parallaxEffect;
